...
mod tests {
    ...
    fn write_varint_works() {
        ...
        crate::write_varint_in_place(&mut vec, 0);
        assert_eq!(vec, vec! [0x00]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 1);
        assert_eq!(vec, vec! [0x01]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 2);
        assert_eq!(vec, vec! [0x02]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 127);
        assert_eq!(vec, vec! [0x7f]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 128);
        assert_eq!(vec, vec! [0x80, 0x01]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 255);
        assert_eq!(vec, vec! [0xff, 0x01]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 25565);
        assert_eq!(vec, vec! [0xdd, 0xc7, 0x01]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 2097151);
        assert_eq!(vec, vec! [0xff, 0xff, 0x7f]);

        vec = vec![];
        crate::write_varint_in_place(&mut vec, 2147483647);
        ...
        vec = vec![];
        crate::write_varint_in_place(&mut vec, -2147483648);
    ...
    fn write_varlong_works() {
        let mut vec: Vec<u8> = vec![];
        crate::write_varlong_in_place(&mut vec, 0);
        assert_eq!(vec, vec! [0x00]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, 1);
        assert_eq!(vec, vec! [0x01]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, 2);
        assert_eq!(vec, vec! [0x02]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, 127);
        assert_eq!(vec, vec! [0x7f]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, 128);
        assert_eq!(vec, vec! [0x80, 0x01]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, 255);
        assert_eq!(vec, vec! [0xff, 0x01]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, 2147483647);
        assert_eq!(vec, vec! [0xff, 0xff, 0xff, 0xff, 0x07]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, 9223372036854775807);
        ...
        vec = vec![];
        crate::write_varlong_in_place(&mut vec, -2147483648);
        assert_eq!(vec, vec! [0x80, 0x80, 0x80, 0x80, 0xf8, 0xff, 0xff, 0xff, 0xff, 0x01]);

        vec = vec![];
        crate::write_varlong_in_place(&mut vec, -9223372036854775808);
